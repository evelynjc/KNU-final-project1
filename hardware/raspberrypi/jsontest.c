...
{
    int              fd, c, ret, lines, cooked_baud = cook_baud(DEFAULT_BAUDRATE);
    ...
    char dataset, jdata[100];
    FILE *fjSource;
    FILE * jp[3];
    char * pID[3] = {"./P1911280001.json", "./P1911280002.json", "./P1911280003.json"};
	...
	while(!stop){
	jSource = open("./jsource.txt", O_WRONLY | O_TRUNC | O_CREAT, 0644);
        for(lines = 0; lines<1600 && !stop; lines++)
        {
            read(fd,&dataset,1); /* modem */
            write(1,&dataset,1); /* stdout */
	    write(jSource,&dataset, 1);

        }
	close(jSource);
	fjSource = fopen("./jsource.txt", "r");
	for(int i = 0;i<3;i++){
		jp[i] = fopen(pID[i], "w");
		fprintf(jp[i], "[\n");
	}

	for(int i = 0;i<27 && i<lines/50;i++){
		fgets(jdata, sizeof(jdata), fjSource);
		strcat(jdata, ",");
		fprintf(jp[i%3], "%s\n", jdata);
...
//	ret = system("curl -F jsonfile=@P1911280001.json http://ec2-54-86-173-196.compute-1.amazonaws.com:3000/upload");
//      printf("p1: %d\n", ret);
//      ret = system("curl -F jsonfile=@P1911280002.json http://ec2-54-86-173-196.compute-1.amazonaws.com:3000/upload");
//      printf("p2: %d\n", ret);
//      ret = system("curl -F jsonfile=@P1911280003.json http://ec2-54-86-173-196.compute-1.amazonaws.com:3000/upload");
//      printf("p3: %d\n", ret);
        sleep(5);
